...
meses = {0 : "%01/20", 1 : "%01/20", 2 : "%03/20", 3 : "%04/20", 4 : "%05/20", 5 : "%06/20", 6 : "%07/20", 7 : "%08/20", 8 : "%09/20", 9 : "%10/20", 10 : "%11/20", 11 : "%12/20"}
...
def Grafica():
    ...
    for i in range(len(meses_s)):
        datos_meses.append(meses_s[i])
    print(datos_meses)
    print(datos)
    plt.plot(datos_meses, datos)
    plt.title("Ventas IsoQuim 2020")
    plt.xlabel("Meses")
...
def Comision():
    ...
    con = sqlite3.connect("costos.db")
    cursor = con.cursor()
    cursor.execute("SELECT Entra FROM Flujo WHERE Operacion=? AND Fecha LIKE ?",("Venta","%12/20",))
    info = cursor.fetchall()
    #print(info)
    con.close()
    for i in range(len(info)):
